...
HPM_IMP(void, hpm_quat_multi_quatfv, const hpmquatf* larg, const hpmquatf* rarg, hpmquatf* out) {
	...
	// TODO resolve swizzle, currently, this does not work!
	const hpmquatf lwwww = vextq_f32(*larg, *larg, 0); 	/*	{ w, x, y, z } => { w, w, w, w }	*/
	const hpmquatf rwxyz = vextq_f32(*rarg, *rarg, 2); 	/*	{ w, x, y, z } => { w, x, y, z }	*/
	const hpmquatf lxxxx = vextq_f32(*larg, *larg, 1); 	/*	{ w, x, y, z } => { x, x, x, x }	*/
	const hpmquatf rxwzy = vextq_f32(*rarg, *rarg, 2); 	/*	{ w, x, y, z } => { x, w, z, y }	*/
	const hpmquatf lyyyy = vextq_f32(*larg, *larg, 2); 	/*	{ w, x, y, z } => { y, y, y, y }	*/
	const hpmquatf ryzwx = vextq_f32(*rarg, *rarg, 3);	/*	{ w, x, y, z } => { y, z, w, x }	*/
	const hpmquatf lzzzz = vextq_f32(*larg, *larg, 3);	/*	{ w, x, y, z } => { z, z, z, z }	*/
	const hpmquatf rzyxw = vextq_f32(*rarg, *rarg, 3);	/*	{ w, x, y, z } => { z, y, x, w }	*/

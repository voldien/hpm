...
HPM_IMP(hpmvecf, hpm_vec4_max_compfv, const hpmvec4f* vec){
	...
	hpmvec4f compfv = _mm_max_ss(cevmax,_mm_shuffle_ps(cevmax, cevmax, _MM_SHUFFLE(0, 1, 3, 2) ));
...
HPM_IMP(hpmvecf, hpm_vec4_min_compfv, const hpmvec4f* vec){
	...
	hpmvec4f compfv = _mm_min_ss(cevmax,_mm_shuffle_ps(cevmax, cevmax, _MM_SHUFFLE(0, 1, 3, 2) ));
#ifdef defined(__SSE3__)
	return _mm_cvtss_f32(compfv);
#else
	return compfv[0];

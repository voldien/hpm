...
HPM_IMP(hpmvecf, hpm_vec4_dotfv, const hpmvec4f *larg, const hpmvec4f *rarg) {
	return hpm_vec4_getxf(*larg) * hpm_vec4_getxf(*rarg) + hpm_vec4_getyf(*larg) * hpm_vec4_getyf(*rarg) +
		   hpm_vec4_getzf(*larg) * hpm_vec4_getzf(*rarg) + hpm_vec4_getwf(*larg) * hpm_vec4_getwf(*rarg);
}

HPM_IMP(hpmvecf, hpm_vec4_lengthfv, const hpmvec4f *arg) {
	return sqrtf(hpm_vec4_getxf(*arg) * hpm_vec4_getxf(*arg) + hpm_vec4_getyf(*arg) * hpm_vec4_getyf(*arg) +
				 hpm_vec4_getzf(*arg) * hpm_vec4_getzf(*arg) + hpm_vec4_getwf(*arg) * hpm_vec4_getwf(*arg));
}

HPM_IMP(hpmvecf, hpm_vec4_lengthsqurefv, const hpmvec4f *arg) {
	return hpm_vec4_getxf(*arg) * hpm_vec4_getxf(*arg) + hpm_vec4_getyf(*arg) * hpm_vec4_getyf(*arg) +
		   hpm_vec4_getzf(*arg) * hpm_vec4_getzf(*arg) + hpm_vec4_getwf(*arg) * hpm_vec4_getwf(*arg);
}
